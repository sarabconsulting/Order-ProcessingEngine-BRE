...
<body>
    <form id="form1" runat="server" style="margin: 20px;">
        ...
        <div id="Authorized" class="row" runat="server">
            ...
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                <h3 style="background: #eee; padding: 10px; border-radius: 3px;">Order History</h3>
                <asp:Panel ID="MainPanel" runat="server" ScrollBars="Vertical" Height="600px">
                    <table class="table table-responsive table-condensed">
                        <thead style="font-size: small;">
                            <tr>
                                <th>OrderId
                        ...
                        <tbody style="font-size: small;">
                            <%--        <asp:Repeater ID="RecordRepeater" runat="server" DataSourceID="PatientRecordDataSource">
                            <ItemTemplate>
                                <tr>
                                    <td>
                                        <asp:Button ID="EditButton" runat="server" Text="Edit" CssClass="btn btn-warning btn-xs"
                                            OnCommand="EditButton_Command" CommandName="EditRecord" CommandArgument='<%#Eval("RecordId")%>'
                                            Visible='<%# Convert.ToBoolean(AllowEdit(Eval("RecordId")))%>' />
                                    </td>
                                    <td>
                                        <%#Eval("LastUpdatedBy")%>
                                    </td>
                                    <td>
                                        <%# Convert.ToDateTime(Eval("RecordDated")).ToShortDateString()%>
                                        <br />
                                    </td>
                                    <td>
                                        <%#Eval("Time")%>
                                    </td>
                                    <td style="background: <%#VitalColor(Eval("BodyWeight"), VitalType.BodyWeight) %>;">
                                        <%#Eval("BodyWeight")%>
                                    </td>
                                    <td style="background: <%#VitalColor(Eval("BPUpper"), VitalType.BOUpper) %>;">
                                        <%#Eval("BPUpper")%>
                                    </td>
                                    <td style="background: <%#VitalColor(Eval("BPLower"), VitalType.BPLower) %>;">
                                        <%#Eval("BPLower")%>
                                    </td>
                                    <td style="background: <%#VitalColor(Eval("BPLower"), VitalType.Pulse) %>;">
                                        <%#Eval("Pulse")%>
                                    </td>
                                    <td style="background: <%#VitalColor(Eval("Sugar"), VitalType.Sugar) %>;">
                                        <%#Eval("Sugar")%>
                                    </td>
                                    <td style="background: <%#VitalColor(Eval("Insulin"), VitalType.Insulin) %>;">
                                        <%#Eval("Insulin")%>
                                    </td>
                                    <td style="background: <%#VitalColor(Eval("WaterIn"), VitalType.WaterIn) %>;">
                                        <%#Eval("WaterIn")%>
                                    </td>
                                    <td style="background: <%#VitalColor(Eval("UrineOut"), VitalType.UrineOut) %>;">
                                        <%#Eval("UrineOut")%>
                                    </td>
                                    <td style="background: <%#VitalColor(Eval("MedicinesTaken"), VitalType.Medication) %>;">
                                        <%#Eval("MedicinesTaken")%>
                                    </td>
                                    <td style="background: <%#VitalColor(Eval("Observations"), VitalType.Observation) %>;">
                                        <%#Eval("Observations")%>
                                    </td>
                                    <td>
                                        <%#Eval("LastUpdatedOnDate")%>
                                        <br />
                                        (RID<%#Eval("RecordId")%>)
                                    </td>
                                </tr>
                ...
                <%-- <asp:SqlDataSource ID="PatientRecordDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:PatientRecordConnectionString %>"
                SelectCommand="SELECT RecordId, UserId, RecordDated, Time, BodyWeight, BPUpper, BPLower, Pulse, Sugar, Insulin, WaterIn, UrineOut, MedicinesTaken, Observations, LastUpdatedOnDate, LastUpdatedBy, RecordRemarks, IsMarked, MarkingReason, IsDeactivated, DeactivationDate FROM MCV1_PatientRecord WHERE (IsDeactivated &lt;&gt; 1) ORDER BY RecordDated DESC">
            </asp:SqlDataSource>--%>
            </div>
        </div>
